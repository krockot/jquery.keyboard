/*
   ...
   Specifically, here are the semantics of a keyboard event e:
	  ...
	  and is comparable to a named entry in $.Keys (i.e., you can test e.key == $.Keys.Escape).
	...
	- If a sane and valid character representation is not in e.chr, e.chr is guaranteed
	  to be null.
	- e.chr is affected by the shift modifier.  An event with e.key == $.Keys.Key0 and
	  e.shift == true will hold e.chr == ')'.
	- e.alt, e.ctrl, and e.shift are always accurate.
	- Most keys and key combinations will fire repeated keyboard events when held down.*
